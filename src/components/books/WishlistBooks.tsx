...
export default function WishlistBooks() {
  ...
  async function fetchBooks() {
    ...
    setLoading(true);
    let query = supabase.from("books").select("*", { count: "exact" }).eq("user_id", session.user.id).eq("wishlist", true);
        ...
        <table className="table w-full">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="text-center">Aucun livre trouvé</td></tr>
            ) : (
